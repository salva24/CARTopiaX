...
void SimParam::LoadParams(const std::string& filename) {
    ...
    if (jfile.contains("kOutputPerformanceStatistics")) {
        kOutputPerformanceStatistics = jfile["kOutputPerformanceStatistics"].get<bool>();
    } else {
    ...
    if (jfile.contains("kVolumeRelaxationRateCytoplasmApoptotic")) {
        kVolumeRelaxationRateCytoplasmApoptotic = jfile["kVolumeRelaxationRateCytoplasmApoptotic"].get<double>();
    } else {
        kVolumeRelaxationRateCytoplasmApoptotic = 1.0 / 60.0;
    }

    if (jfile.contains("kVolumeRelaxationRateNucleusApoptotic")) {
        kVolumeRelaxationRateNucleusApoptotic = jfile["kVolumeRelaxationRateNucleusApoptotic"].get<double>();
    } else {
        kVolumeRelaxationRateNucleusApoptotic = 0.35 / 60.0;
    }

    if (jfile.contains("kVolumeRelaxationRateFluidApoptotic")) {
        kVolumeRelaxationRateFluidApoptotic = jfile["kVolumeRelaxationRateFluidApoptotic"].get<double>();
    } else {
    ...
    if (jfile.contains("kReductionConsumptionDeadCells")) {
        kReductionConsumptionDeadCells = jfile["kReductionConsumptionDeadCells"].get<double>();
    } else {
    ...
    if (jfile.contains("kDiffusionCoefficientOxygen")) {
        kDiffusionCoefficientOxygen = jfile["kDiffusionCoefficientOxygen"].get<double>();
    } else {
    ...
    if (jfile.contains("kDiffusionCoefficientImmunostimulatoryFactor")) {
        kDiffusionCoefficientImmunostimulatoryFactor = jfile["kDiffusionCoefficientImmunostimulatoryFactor"].get<double>();
    } else {
        kDiffusionCoefficientImmunostimulatoryFactor = 1000;
    }

    if (jfile.contains("kDecayConstantImmunostimulatoryFactor")) {
        kDecayConstantImmunostimulatoryFactor = jfile["kDecayConstantImmunostimulatoryFactor"].get<double>();
    } else {
    ...
    if (jfile.contains("kMaxRelativeAdhesionDistance")) {
        kMaxRelativeAdhesionDistance = jfile["kMaxRelativeAdhesionDistance"].get<double>();
    } else {
    ...
    if (jfile.contains("kRateSecretionImmunostimulatoryFactor")) {
        kRateSecretionImmunostimulatoryFactor = jfile["kRateSecretionImmunostimulatoryFactor"].get<double>();
    } else {
        kRateSecretionImmunostimulatoryFactor = 10.0;
    }

    if (jfile.contains("kSaturationDensityImmunostimulatoryFactor")) {
        kSaturationDensityImmunostimulatoryFactor = jfile["kSaturationDensityImmunostimulatoryFactor"].get<double>();
    } else {
    ...
    if (jfile.contains("kOncoproteinStandardDeviation")) {
        kOncoproteinStandardDeviation = jfile["kOncoproteinStandardDeviation"].get<double>();
    } else {
        kOncoproteinStandardDeviation = 0.25;
    }

    if (jfile.contains("kOxygenSaturationInProliferation")) {
        kOxygenSaturationInProliferation = jfile["kOxygenSaturationInProliferation"].get<double>();
    } else {
        kOxygenSaturationInProliferation = 38.0;
    }

    if (jfile.contains("kOxygenLimitForProliferation")) {
        kOxygenLimitForProliferation = jfile["kOxygenLimitForProliferation"].get<double>();
    } else {
    ...
    if (jfile.contains("kOxygenLimitForNecrosisMaximum")) {
        kOxygenLimitForNecrosisMaximum = jfile["kOxygenLimitForNecrosisMaximum"].get<double>();
    } else {
    ...
    if (jfile.contains("kDefaultOxygenConsumptionTumorCell")) {
        kDefaultOxygenConsumptionTumorCell = jfile["kDefaultOxygenConsumptionTumorCell"].get<double>();
    } else {
        kDefaultOxygenConsumptionTumorCell = 10.0;
    }

    if (jfile.contains("kDefaultVolumeNewTumorCell")) {
        kDefaultVolumeNewTumorCell = jfile["kDefaultVolumeNewTumorCell"].get<double>();
    } else {
        kDefaultVolumeNewTumorCell = 2494.0;
    }

    if (jfile.contains("kDefaultVolumeNucleusTumorCell")) {
        kDefaultVolumeNucleusTumorCell = jfile["kDefaultVolumeNucleusTumorCell"].get<double>();
    } else {
        kDefaultVolumeNucleusTumorCell = 540.0;
    }

    if (jfile.contains("kDefaultFractionFluidTumorCell")) {
        kDefaultFractionFluidTumorCell = jfile["kDefaultFractionFluidTumorCell"].get<double>();
    } else {
        kDefaultFractionFluidTumorCell = 0.75;
    }

    if (jfile.contains("kAverageTimeTransformationRandomRate")) {
        kAverageTimeTransformationRandomRate = jfile["kAverageTimeTransformationRandomRate"].get<double>();
    } else {
        kAverageTimeTransformationRandomRate = 38.6;
    }

    if (jfile.contains("kStandardDeviationTransformationRandomRate")) {
        kStandardDeviationTransformationRandomRate = jfile["kStandardDeviationTransformationRandomRate"].get<double>();
    } else {
    ...
    if (jfile.contains("kMinimumDistanceCarTFromTumor")) {
        kMinimumDistanceCarTFromTumor = jfile["kMinimumDistanceCarTFromTumor"].get<double>();
    } else {
...
void SimParam::PrintParams() const {
    ...
    std::cout << "Output Performance Statistics: " << (kOutputPerformanceStatistics ? "true" : "false") << "\n";
    std::cout << "Total simulation time in minutes (30 days): " << kTotalMinutesToSimulate << "\n";
    std::cout << "Initial radius of the spherical tumor (micrometers): " << kInitialRadiusTumor << "\n";
    std::cout << "Length of the bounded space (micrometers): " << kBoundedSpaceLength << "\n\n";
    ...
    std::cout << "/// Time steps\n";
    std::cout << "///\n\n";
    std::cout << "Time step for substances secretion/consumption (minutes): " << kDtSubstances << "\n";
    ...
    std::cout << "///\n\n";
    std::cout << "Volume relaxation rate cytoplasm apoptotic: " << kVolumeRelaxationRateCytoplasmApoptotic << "\n";
    std::cout << "Volume relaxation rate nucleus apoptotic: " << kVolumeRelaxationRateNucleusApoptotic << "\n";
    std::cout << "Volume relaxation rate fluid apoptotic: " << kVolumeRelaxationRateFluidApoptotic << "\n";
    std::cout << "Time in minutes until an apoptotic cell is removed: " << kTimeApoptosis << "\n";
    std::cout << "Reduction of consumption rate of dead cells when they enter necrosis: " << kReductionConsumptionDeadCells << "\n\n";

    /// Chemicals
    ///
    std::cout << "/// Chemicals\n";
    std::cout << "///\n\n";
    std::cout << "Number of voxels per axis for the substances grid: " << kResolutionGridSubstances << "\n";
    std::cout << "Diffusion coefficient of oxygen (μm²/min): " << kDiffusionCoefficientOxygen << "\n";
    std::cout << "Decay constant of oxygen (min⁻¹): " << kDecayConstantOxygen << "\n";
    std::cout << "Diffusion coefficient of immunostimulatory factor (μm²/min): " << kDiffusionCoefficientImmunostimulatoryFactor << "\n";
    std::cout << "Decay constant of immunostimulatory factor (min⁻¹): " << kDecayConstantImmunostimulatoryFactor << "\n";
    std::cout << "Reference level of oxygen at the boundaries (mmHg): " << kOxygenReferenceLevel << "\n";
    std::cout << "Initial oxygen concentration in each voxel (mmHg): " << kInitialOxygenLevel << "\n";
    std::cout << "Oxygen saturation in the microenvironment (mmHg): " << kOxygenSaturation << "\n\n";

    /// Forces
    ///
    std::cout << "/// Forces\n";
    std::cout << "///\n\n";
    std::cout << "Repulsion coefficient between tumor cells: " << kRepulsionTumorTumor << "\n";
    std::cout << "Repulsion coefficient between CAR-T cells: " << kRepulsionCartCart << "\n";
    std::cout << "Repulsion coefficient between CAR-T cells and tumor cells: " << kRepulsionCartTumor << "\n";
    std::cout << "Repulsion coefficient between tumor cells and CAR-T cells: " << kRepulsionTumorCart << "\n";
    std::cout << "Maximum relative adhesion distance for cell interactions: " << kMaxRelativeAdhesionDistance << "\n";
    std::cout << "Adhesion coefficient between tumor cells: " << kAdhesionTumorTumor << "\n";
    std::cout << "Adhesion coefficient between CAR-T cells: " << kAdhesionCartCart << "\n";
    std::cout << "Adhesion coefficient between CAR-T cells and tumor cells: " << kAdhesionCartTumor << "\n";
    std::cout << "Adhesion coefficient between tumor cells and CAR-T cells: " << kAdhesionTumorCart << "\n";
    std::cout << "Box length for mechanics calculations (micrometers): " << kLengthBoxMechanics << "\n";
    std::cout << "Coefficient for the current time step in Adams-Bashforth method: " << kDnew << "\n";
    std::cout << "Coefficient for the previous time step in Adams-Bashforth method: " << kDold << "\n";
    std::cout << "Maximum squared distance to avoid CAR-T pushing tumor cells: " << kMaxSquaredDistanceCartMovingTowardsTumorCell << "\n\n";
    ...
    std::cout << "Rate of secretion of immunostimulatory factor of tumor cells per minute: " << kRateSecretionImmunostimulatoryFactor << "\n";
    std::cout << "Saturation density of immunostimulatory factor for tumor cells: " << kSaturationDensityImmunostimulatoryFactor << "\n";
    std::cout << "Mean level of oncoprotein expression in tumor cells: " << kOncoproteinMean << "\n";
    std::cout << "Standard deviation of oncoprotein expression in tumor cells: " << kOncoproteinStandardDeviation << "\n";
    std::cout << "Oxygen saturation level in tumor cells for proliferation: " << kOxygenSaturationInProliferation << "\n";
    std::cout << "Limit of oxygen level for tumor cell proliferation: " << kOxygenLimitForProliferation << "\n";
    std::cout << "Limit of oxygen to start causing necrosis: " << kOxygenLimitForNecrosis << "\n";
    std::cout << "Limit of oxygen to maximum necrosis probability: " << kOxygenLimitForNecrosisMaximum << "\n";
    std::cout << "Time in minutes until a lysed necrotic cell is removed: " << kTimeLysis << "\n";
    std::cout << "Maximum rate per minute of necrosis for tumor cells: " << kMaximumNecrosisRate << "\n";
    std::cout << "Default oxygen consumption rate of tumor cell: " << kDefaultOxygenConsumptionTumorCell << "\n";

    std::cout << "\nVolume parameters:\n";
    std::cout << "Default total volume of a new tumor cell (μm³): " << kDefaultVolumeNewTumorCell << "\n";
    std::cout << "Default volume of the nucleus of a new tumor cell (μm³): " << kDefaultVolumeNucleusTumorCell << "\n";
    std::cout << "Default fraction of fluid volume in a new tumor cell: " << kDefaultFractionFluidTumorCell << "\n";
    
    std::cout << "\nTransformation and adhesion parameters:\n";
    std::cout << "Average time for transformation Random Rate (hours): " << kAverageTimeTransformationRandomRate << "\n";
    std::cout << "Standard Deviation for transformation Random Rate (hours): " << kStandardDeviationTransformationRandomRate << "\n";
    std::cout << "Average adhesion time for Tumor Cell under CAR-T attack (minutes): " << kAdhesionTime << "\n";
    std::cout << "Min oncoprotein level to be killed by a CAR-T cell: " << kOncoproteinLimit << "\n";
    std::cout << "Max oncoprotein level: " << kOncoproteinSaturation << "\n";
    std::cout << "Oncoprotein difference (saturation - limit): " << kOncoproteinDifference << "\n";

    std::cout << "\nVolume relaxation rates for alive cells:\n";
    std::cout << "Volume relaxation rate alive cytoplasm (min⁻¹): " << kVolumeRelaxationRateAliveCytoplasm << "\n";
    std::cout << "Volume relaxation rate alive nucleus (min⁻¹): " << kVolumeRelaxationRateAliveNucleus << "\n";
    std::cout << "Volume relaxation rate alive fluid (min⁻¹): " << kVolumeRelaxationRateAliveFluid << "\n";

    std::cout << "\nVolume relaxation rates for necrotic swelling:\n";
    std::cout << "Volume relaxation rate cytoplasm necrotic swelling (min⁻¹): " << kVolumeRelaxationRateCytoplasmNecroticSwelling << "\n";
    std::cout << "Volume relaxation rate nucleus necrotic swelling (min⁻¹): " << kVolumeRelaxationRateNucleusNecroticSwelling << "\n";
    std::cout << "Volume relaxation rate fluid necrotic swelling (min⁻¹): " << kVolumeRelaxationRateFluidNecroticSwelling << "\n";

    std::cout << "\nVolume relaxation rates for necrotic lysed:\n";
    std::cout << "Volume relaxation rate cytoplasm necrotic lysed (min⁻¹): " << kVolumeRelaxationRateCytoplasmNecroticLysed << "\n";
    std::cout << "Volume relaxation rate nucleus necrotic lysed (min⁻¹): " << kVolumeRelaxationRateNucleusNecroticLysed << "\n";
    std::cout << "Volume relaxation rate fluid necrotic lysed (min⁻¹): " << kVolumeRelaxationRateFluidNecroticLysed << "\n";

    std::cout << "\nThresholds in oncoprotein levels for differentiating cancer cell types:\n";
    ...
    std::cout << "Average time in minutes until a CAR-T cell dies: " << kAverageMaximumTimeUntillApoptosisCart << "\n";
    std::cout << "Default oxygen consumption rate of CAR-T cell: " << kDefaultOxygenConsumptionCarT << "\n";
    
    std::cout << "\nVolume parameters:\n";
    std::cout << "Default total volume of a new CAR-T cell (μm³): " << kDefaultVolumeNewCarTCell << "\n";

    std::cout << "\nKilling and adhesion rates:\n";
    std::cout << "How often a CAR-T cell tries to kill an attached cancer cell (1/min): " << kKillRateCart << "\n";
    std::cout << "How often a CAR-T cell tries to attach to a cancer cell (1/min): " << kAdhesionRateCart << "\n";
    std::cout << "Maximum adhesion distance between CAR-T and tumor cells (micrometers): " << kMaxAdhesionDistanceCart << "\n";
    std::cout << "Minimum adhesion distance between CAR-T and tumor cells (micrometers): " << kMinAdhesionDistanceCart << "\n";

    std::cout << "\nSpawning parameters:\n";
    std::cout << "Minimum distance from the tumor for spawning CAR-T cells (micrometers): " << kMinimumDistanceCarTFromTumor << "\n";

    std::cout << "\nMotility parameters:\n";
    std::cout << "Average persistence time before CAR-T cell moves: " << kPersistenceTimeCart << "\n";
    std::cout << "Migration bias (higher values = more directed movement): " << kMigrationBiasCart << "\n";
