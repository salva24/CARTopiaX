...
struct SimParam : public ParamGroup {
  BDM_PARAM_GROUP_HEADER(SimParam, 1);
    ...
    bool kOutputPerformanceStatistics = true;
    /// Total simulation time in minutes (30 days)
    int kTotalMinutesToSimulate = 30 * 24 * 60;
    ...
    std::map<size_t, size_t> kTreatment = {};
    ...
    int kOutputCsvInterval = 1;

    /// Apoptotic cells volume change
    real_t kVolumeRelaxationRateCytoplasmApoptotic = 1.0 / 60.0;
    real_t kVolumeRelaxationRateNucleusApoptotic = 0.35 / 60.0;
    real_t kVolumeRelaxationRateFluidApoptotic = 0.0;
    /// Time in minutes until an apoptotic cell is removed from the simulation
    real_t kTimeApoptosis = 8.6 * 60;
    ...
    real_t kMaxSquaredDistanceCartMovingTowardsTumorCell = 0;
    ...
    real_t kRateSecretionImmunostimulatoryFactor = 10.0;
    /// Saturation density of immunostimulatory factor for tumor cells
    real_t kSaturationDensityImmunostimulatoryFactor = 1.0;
    /// Mean level of oncoprotein expression in tumor cells
    real_t kOncoproteinMean = 1.0;
    ...
    real_t kOxygenSaturationInProliferation = 38.0;
    /// Limit of oxygen level for tumor cell proliferation
    real_t kOxygenLimitForProliferation = 10.0;
    /// Limit of oxygen to start causing necrosis
    real_t kOxygenLimitForNecrosis = 5.0;
    ...
    real_t kOncoproteinDifference = 0;

    /// volume relaxation rate (min^-1) for each state
    real_t kVolumeRelaxationRateAliveCytoplasm = 0.13 / 60.;
    real_t kVolumeRelaxationRateAliveNucleus = 0.22 / 60.;
    real_t kVolumeRelaxationRateAliveFluid = 1.3 / 60.;

    real_t kVolumeRelaxationRateCytoplasmNecroticSwelling = 0.0032 / 60.0;
    real_t kVolumeRelaxationRateNucleusNecroticSwelling = 0.013 / 60.;
    real_t kVolumeRelaxationRateFluidNecroticSwelling = 0.050 / 60.0;

    real_t kVolumeRelaxationRateCytoplasmNecroticLysed = 0.0032 / 60.00;
    real_t kVolumeRelaxationRateNucleusNecroticLysed = 0.013 / 60.;
    real_t kVolumeRelaxationRateFluidNecroticLysed = 0.050 / 60.0;
    ...
    real_t kAverageMaximumTimeUntillApoptosisCart = 0;
    /// Default oxygen consumption rate of CAR-T cell
    real_t kDefaultOxygenConsumptionCarT = 1.0;
    /// Volume parameters
    ///  Default total volume of a new CAR-T cell in μm³
    real_t kDefaultVolumeNewCarTCell = 2494.0;
    ...
    /// Migration speed
    real_t kMigrationSpeedCart = 5.0;
    /// Elastic constant
    ...
    /// fmod
    int kStepsPerCycle = 0;
    /// Steps in one day
    size_t kStepsOneDay = 1;
    /// Volume of a single voxel in μm³
    real_t kVoxelVolume = 0;
    /// 1-kMigrationBiasCart
    real_t kMigrationOneMinusBiasCart = 0;
    /// Probability of a CAR-T cell to migrate in a given
    /// mechanical time step
    real_t kMotilityProbability = 0;
    /// Probability of a Tumor cell to escape in a given
    /// mechanical time step
    real_t kProbabilityEscape = 0;

    /// Maximum adhesion distance squared
    real_t kSquaredMaxAdhesionDistanceCart = 0;
    /// Difference between min and max adhesion distance
    real_t kDifferenceCartAdhesionDistances = 0;

    /// Radius tumor cell
    real_t kRadiusTumorCell = 0;

    /// Radius cart cell
    real_t kRadiusCarTCell = 0;
    ...
    real_t kSquaredMaxDistanceNeighborsForce = 0;
